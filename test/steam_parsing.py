...
def extract_author(soup):
    ...
    return main_author_div.decode_contents().split("<br/>")[0].strip() if main_author_div else "Unknown Author"
...
def parse_dependencies(soup, noisy=False):
    ...
    for link in dependency_div.find_all("a", href=True):
        ...
        try:
            ...
            dependency_author = author_div.decode_contents().split("<br/>")[0].strip() if author_div else "Unknown Author"
            
            if dependency_id not in [dep["id"] for dep in dependencies]:
                dependencies.append({"id": dependency_id, "name": dependency_name, "author": dependency_author})
                if noisy:
                    print(f"Dependency found: {dependency_name} (ID: {dependency_id}) by {dependency_author}")
...
workshop_id = "3329396988"
result = fetch_dependencies(workshop_id, noisy=True)
